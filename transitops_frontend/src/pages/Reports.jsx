...
// Computed client-side from raw records so Reports works even before B ships
// a dedicated /api/reports/ endpoint. If B does add one, swap this for a
// single fetch — the table below only needs the same shape.
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          rows.length > 0 && (
            <button onClick={() => downloadCSV(rows)} className="rounded-md bg-console-bg px-4 py-2 text-sm font-medium text-white hover:brightness-110">
              Export CSV
            </button>
          )
        }
