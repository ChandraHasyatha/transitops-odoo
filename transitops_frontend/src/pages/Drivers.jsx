        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-400 dark:bg-slate-900 dark:text-slate-500">
              <tr>
                <th
                  className={`cursor-pointer select-none px-4 py-3 hover:text-slate-200 ${sortKey === 'name' ? 'text-slate-200' : ''}`}
                  onClick={() => toggleSort('name')}
                >
                  Name{sortIndicator('name')}
                </th>
                <th
                  className={`cursor-pointer select-none px-4 py-3 hover:text-slate-200 ${sortKey === 'license_number' ? 'text-slate-200' : ''}`}
                  onClick={() => toggleSort('license_number')}
                >
                  License{sortIndicator('license_number')}
                </th>
                <th
                  className={`cursor-pointer select-none px-4 py-3 hover:text-slate-200 ${sortKey === 'license_expiry' ? 'text-slate-200' : ''}`}
                  onClick={() => toggleSort('license_expiry')}
                >
                  Expiry{sortIndicator('license_expiry')}
                </th>
                <th
                  className={`cursor-pointer select-none px-4 py-3 hover:text-slate-200 ${sortKey === 'safety_score' ? 'text-slate-200' : ''}`}
                  onClick={() => toggleSort('safety_score')}
                >
                  Safety Score{sortIndicator('safety_score')}
                </th>
                <th
                  className={`cursor-pointer select-none px-4 py-3 hover:text-slate-200 ${sortKey === 'status' ? 'text-slate-200' : ''}`}
                ...
                return (
                  <tr key={d.id}>
                    ...
                    <td className="px-4 py-3">
                      <span className={expired ? 'text-signal-red font-medium' : expiringSoon ? 'text-signal-amber font-medium' : 'dark:text-slate-300'}>
                        {d.license_expiry}
                      </span>
